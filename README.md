...
`hub-intro` (header block: `hub-kicker`, `pills` + `pill`/`pill active`, `hub-sub`) · `hub-section` (group label) · `hub-band` (grey band) · `hub-cards bg-soft` (soft grey band behind the tile grid) · `project-card` (tile: `project-link`, `project-thumb`, `project-body`, `project-title` + `title-arrow`) · `badge` ("coming soon" chip) · `tags` + `tag` (small topic chips on tiles) · add `coming` to unclickable tiles
